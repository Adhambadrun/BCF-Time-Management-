...
import { Users, Clock, Coffee, ShieldCheck, Activity, TrendingUp, AlertTriangle, CheckCircle2 } from 'lucide-react';
import { GLIDE } from '../../styles/motion-presets';

export interface TeamStatsWidgetProps {
  team: Team;
  isAllTeams?: boolean;
  agents: User[];
  breaks: BreakRecord[];
...
export const TeamStatsWidget: React.FC<TeamStatsWidgetProps> = ({
  ...
  const activeBreakAverageSeconds =
    activeBreaks.length > 0
      ? Math.round(activeBreaks.reduce((sum, b) => sum + (b.duration || 0), 0) / activeBreaks.length)
      : 0;
  ...
  const todayAverageSeconds =
    todayCompletedBreaks.length > 0
      ? Math.round(
          todayCompletedBreaks.reduce((sum, b) => sum + (b.duration || 0), 0) / todayCompletedBreaks.length
        )
      : 0;

  // Overtime breaks count
  const overtimeBreaksCount = activeBreaks.filter(b => (b.duration || 0) > (shiftConfig.maxSlotDuration || 15) * 60).length;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-orbitron font-extrabold text-base sm:text-lg text-zinc-100 uppercase tracking-wide">
                {isAllTeams ? 'All Teams Real-Time KPIs' : `${team.teamName} Performance Metrics`}
              </h2>
            </div>
            <p className="text-xs text-zinc-400 font-inter mt-0.5">
              Live ratio of agents on floor vs on break, average active break durations, and capacity metrics.
            </p>
          </div>
        </div>
        ...
        {/* Right Status Badge */}
        <div className="flex items-center gap-2">
          {overtimeBreaksCount > 0 ? (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-crimson/15 border border-crimson/40 text-crimson text-xs font-orbitron font-bold animate-pulse">
              <AlertTriangle className="w-3.5 h-3.5 text-crimson" />
              <span>{overtimeBreaksCount} Overtime Break(s)</span>
            </div>
          ) : (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        ...
        {/* Metric 2: Average Break Duration */}
        <div className="p-3.5 rounded-2xl bg-white/5 border border-white/10 flex flex-col justify-between">
          ...
          <div className="text-[11px] text-zinc-400 font-inter">
            {activeBreaks.length > 0
              ? `${activeBreaks.length} active break timer(s) in progress`
              : `Based on ${todayCompletedBreaks.length} completed breaks today`}
          </div>
        </div>

        {/* Metric 3: Live Capacity In Use */}
        <div className="p-3.5 rounded-2xl bg-white/5 border border-white/10 flex flex-col justify-between">
          <div className="flex items-center justify-between text-zinc-400 text-xs font-orbitron">
            <span className="flex items-center gap-1.5">
        ...
        {/* Metric 4: Shift Total Logged Break Volume */}
        <div className="p-3.5 rounded-2xl bg-white/5 border border-white/10 flex flex-col justify-between">
          <div className="flex items-center justify-between text-zinc-400 text-xs font-orbitron">
            <span className="flex items-center gap-1.5">
